// VsPkg.cs : Implementation of brim
//
...
namespace noname.brim
{
    ...
    [ProvideLoadKey("Standard", "0.1", "brimLanguageService", "noname", 1)]
    // This attribute is needed to let the shell know that this package exposes some menus.
    [ProvideMenuResource(1000, 1)]
    // This attribute registers a tool window exposed by this package.
    [ProvideToolWindow(typeof(MyToolWindow))]
    [Guid(GuidList.guidbrimPkgString)]
    ...
    public sealed class brim : Package
    {
        ...
        protected override void Initialize()
        {
            ...
            if ( null != mcs )
            {
                ...
                CommandID menuCommandID = new CommandID(GuidList.guidbrimCmdSet, (int)PkgCmdIDList.cmdidMyCommand);
                MenuCommand menuItem = new MenuCommand( new EventHandler(MenuItemCallback), menuCommandID );
                mcs.AddCommand( menuItem );
                // Create the command for the tool window
                CommandID toolwndCommandID = new CommandID(GuidList.guidbrimCmdSet, (int)PkgCmdIDList.cmdidMyTool);
                MenuCommand menuToolWin = new MenuCommand( new EventHandler(ShowToolWindow), toolwndCommandID);
                mcs.AddCommand( menuToolWin );
            }

            _languageService = new Babel.BrimLanguageService();
            _languageService.SetSite(this);

            IServiceContainer serviceContainer = (IServiceContainer)this;
            serviceContainer.AddService(typeof(Babel.LanguageService), _languageService, true);
        }
        #endregion
